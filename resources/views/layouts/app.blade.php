...
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap@5.7.0/dist/css/bootstrap.min.css">
      ...
      <script>
         const baseUrl = "{{ url('/') }}" ;
         var loading_set =
          '<div style="text-align:center;width:100%;position:relative;"><i style="color:black;font-size:25px;" class="fa fa-refresh fa-spin fa-3x fa-fw"></i><p>Please wait</p></div>';
 
       var userLogin = "{{ Session::has('userToken') }}";
      </script>
                  ...
                  <div class="row">
                        ...
                        <div class="detail-gallery">
                           ...
                           <div class="product-image-slider">
                              <figure class="border-radius-10">
                                 <img id="product_fimage" src="" style="height:390px;" alt="product image" />
                              </figure>
                              <figure class="border-radius-10">
                                 <img id="product_simage" src="" style="height:390px;" alt="product image" />
                              </figure>
                           </div>
                           <!-- THUMBNAILS -->
                           <div class="slider-nav-thumbnails">
                              <div><img id="other_fimage" src="" alt="product image" style="height:85px;" /></div>
                              <div><img id="other_simage" src="" alt="product image" style="height:85px;" /></div>
                           </div>
                        </div>
                        <!-- End Gallery -->
                     </div>
                     <div class="col-md-6 col-sm-12 col-xs-12">
                        <div class="detail-info pr-30 pl-30">
                           ...
                           <h3 class="title-detail"><a href="shop-product-right.html" class="text-heading" id="product_name"></a></h3>
                           ...
                           <div class="clearfix product-price-cover">
                              <div class="product-price primary-color float-left">
                                 <span class="current-price text-brand" id="offer_price"></span>
                                 <span>
                                 <span class="save-price font-md color3 ml-15" style="display: inline-block;">
                                    <h6 id="discount_amount" class="save-price font-md color3 ml-15" style="display: inline; margin: 0;"></h6>% Off
                                 </span>
                                 <span class="old-price font-md ml-15" id="sale_price"></span>
                                 </span>
                              </div>
                           </div>
                           <div class="detail-extralink mb-30 add-cart product my-1">

                                 <div class="detail-qty border radius">
                                    <a href="#" class="qty-down" onclick="event.preventDefault();"><i class="fi-rs-angle-small-down"></i></a>
                                    <input type="text" name="quantity" class="qty-val" value="1" min="1" disabled>
                                    <a href="#" class="qty-up" onclick="event.preventDefault();"><i class="fi-rs-angle-small-up"></i></a>                                    
                                 </div>
                                 <div class="product-extra-link2">
                                    <input type="hidden" id="product-id" class="product-id" value="">
                           ...
                           <div class="font-xs">
                              <ul>
                                 ...
                                 <li class="mb-5">Vendor: <span class="text-brand">Nest Mart</span></li>
                              </ul>
                           </div>
                        </div> 
                        <!-- Detail Info -->
                     ...
                     <div class="header-action-right">
                        <div class="header-action-2">
                           ...
                           <div class="header-action-icon-2">
                              <a href="{{route('user.my-wishlists')}}">
                              <img class="svgInject" alt="Nest" src="{{ asset('assets/imgs/theme/icons/icon-heart.svg') }}" />
                              <span class="pro-count blue" id="wishlist-count">0</span>
                              </a>
                              <a href="{{route('user.my-wishlists')}}"><span class="lable">Wishlist</span></a>
                           </div>
                           <div class="header-action-icon-2">
                              <a class="mini-cart-icon1" href="{{ route('cart') }}">
                              <img alt="Nest" src="{{ asset('assets/imgs/theme/icons/icon-cart.svg') }}" />
                              <span class="pro-count blue"id="cart-count">{{ isset($userCartData) && !empty($userCartData) ? count($userCartData) : 0 }}</span>
                              </a>
                           ...
                           @if(Auth::user() && Auth::user()->designation_id == '2')
                           <div class="header-action-icon-2">
                           ...
                           <div class="header-action-icon-2">
                              <a class="mini-cart-icon" href="#">
                                 <img alt="Nest" src="{{ asset('assets/imgs/theme/icons/icon-user.svg') }}" />
                              </a>
                              <a href="{{ url('user-login') }}"><span class="lable">Login</span></a>
   ...
   <script>
      $(document).ready(function () {
         $('#quickViewModal').on('show.bs.modal', function (event) {
               ...
               var roundedDiscount = Math.round(productData.discount_amount * 100) / 100;
               var salePrice = Math.round(productData.sale_price * 100) / 100;               // var formattedDate = new Date(productData.created_at).toISOString().split('T')[0];

               // Populate modal with product details
               $('#product_name').text(productData.name);
               $('#offer_price').html('₹'+productData.offer_price);
               $('#sale_price').text('₹'+salePrice);
               $('#discount_amount').text(roundedDiscount);
               $('#product-id').val(productData.varientProductId);
               $('#product_fimage').attr('src', productData.first_image);
               $('#other_fimage').attr('src', productData.first_image);
               $('#product_simage').attr('src', productData.second_image);
               $('#other_simage').attr('src', productData.second_image);
         });
    ...
    <script>
        $(document).ready(function() {
            $('.mini-cart-icon1').hover(function() {
                // Make an AJAX request to fetch cart content
                
                $.ajax({
                    url: '/get-cart',
                    method: 'GET',
                    success: function(response) {
                        // Update the cart dropdown with the fetched content
