							...
							<tbody>
								@foreach($salesDetail as $key=>$sales)
									<tr>
										...
										@if ($sales->order_status == '8')
										<td>
											{{$sales->return_remark}}
										</td>
										@endif
										@if ($sales->order_status == '8' && $sales->return_status=='Pending')
										<td>
											<a href="{{url('admin/sales/return-approve',$sales->id)}}" data-type="approve"  data-id="{{$sales['id']}}" title="Approved Order" class="btn btn-tbl-edit">
												<i class="fas fa-check"></i>
											</a>
											<a href="javascript:void(0)" data-type="reject"  title="Reject Order"  data-type="reject"  data-id="{{$sales['id']}}" data-bs-target="exampleModal" class="btn btn-tbl-delete returnreject" >
												<i class="fas fa-times"></i>
											</a>
										</td>
										@elseif($sales->order_status == '8' && $sales->return_status=='Approved')
										<td>
											<div class="badge col-green">{{ $sales->return_status}}</div>
										</td>
										@elseif($sales->order_status == '8' && $sales->return_status=='Rejected')
										<td>
											<div class="badge col-red">{{ $sales->return_status}}</div>
										</td>
										<td>
											{{ $sales->reject_remark}}
										</td>
